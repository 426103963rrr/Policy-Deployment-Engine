...
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `source` | The ID of the SCC source to which the IAM policy is attached. | true | true | Specifies the SCC source being secured. Incorrect or missing source values could result in IAM bindings not being applied to the intended findings source. | 5678 |  |
...
| `policy_data` | An IAM policy data source to define multiple bindings at once. | false | true | When used, policy_data ensures consistent IAM application. If misconfigured, it can unintentionally override other bindings. | data.google_iam_policy.scc_policy |  |
