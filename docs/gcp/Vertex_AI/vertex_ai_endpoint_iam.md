## 🛡️ Policy Deployment Engine: `vertex_ai_endpoint_iam`

This section provides a concise policy evaluation for the `vertex_ai_endpoint_iam` resource in GCP.

Reference: [Terraform Registry – vertex_ai_endpoint_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/vertex_ai_endpoint_iam)
...
| `endpoint` |  | false | false | None | None | None |
...
| `role` | `google_vertex_ai_endpoint_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
